...
def plot2dhist_BATS(esper_sel, esper_type, fig, ax, subplot_label, colorbar_flag):
    ...
    frac_above_zero = len(del_alk[del_alk > 0])/len(del_alk) * 100
    print('% of data above zero: ' + str(round(frac_above_zero,2)) + '%')
    ax.text(esper_sel['decy'].min() + 0.3, 51, 'OLS: $m={:+.3f}$, $p={:.1e}$'.format(ols_results.params[1],ols_results.pvalues[1]), fontsize=10) # for LIR-trained only
    ax.text(esper_sel['decy'].min() + 0.3, 42, 'RLM: $m={:+.3f}$, $p={:.1e}$'.format(rlm_results.params[1],rlm_results.pvalues[1]), fontsize=10) # for LIR-trained only
    ax.text(esper_sel['decy'].min() + 0.3, -55, subplot_label, fontsize=12) # for LIR-trained only
...
def plot2dhist_S_HOT(esper_sel, esper_type, fig, ax, subplot_label, colorbar_flag):
    # sort by time
    esper_sel = esper_sel.sort_values(by=['dectime'],ascending=True)
